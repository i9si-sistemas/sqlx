...
func TestCompileQuery(t *testing.T) {
	...
	for _, test := range table {
		qr, names, err := compileNamedQuery([]byte(test.Q), binder.QUESTION)
		if err != nil {
			t.Error(err)
		}
		if qr != test.R {
		...
		qd, _, _ := compileNamedQuery([]byte(test.Q), binder.DOLLAR)
		if qd != test.D {
			t.Errorf("\nexpected: `%s`\ngot:      `%s`", test.D, qd)
		}

		qt, _, _ := compileNamedQuery([]byte(test.Q), binder.AT)
		if qt != test.T {
			t.Errorf("\nexpected: `%s`\ngot:      `%s`", test.T, qt)
		}

		qq, _, _ := compileNamedQuery([]byte(test.Q), binder.NAMED)
		if qq != test.N {
...
func TestEscapedColons(t *testing.T) {
	...
	_, _, err := compileNamedQuery([]byte(qs), binder.DOLLAR)
	if err != nil {
