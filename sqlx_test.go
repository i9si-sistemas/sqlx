...
func TestRebind(t *testing.T) {
	...
	s1 := defaultBinder.Rebind(binder.DOLLAR, q1)
	s2 := defaultBinder.Rebind(binder.DOLLAR, q2)
	...
	defaultBinder = binder.Default
	s1 = defaultBinder.Rebind(binder.AT, q1)
	s2 = defaultBinder.Rebind(binder.AT, q2)
	...
	s1 = defaultBinder.Rebind(binder.NAMED, q1)
	s2 = defaultBinder.Rebind(binder.NAMED, q2)
...
func TestBindMap(t *testing.T) {
	...
	bq, args, _ := bindMap(binder.QUESTION, q1, am)
...
func TestIn(t *testing.T) {
	...
	for _, test := range tests {
		q, a, err := binder.Default.In(test.q, test.args...)
		if err != nil {
			t.Error(err)
		}
		if len(a) != test.c {
	...
	// might not work, but we shouldn't parse if we don't need to
	{
		...
		q, a, err := binder.Default.In(orig, "foo", "bar", "baz")
		if err != nil {
			t.Error(err)
		}
		if len(a) != 3 {
	...
	for _, test := range tests {
		_, _, err := binder.Default.In(test.q, test.args...)
		if err == nil {
	...
	RunWithSchema(defaultSchema, t, func(db *DB, t *testing.T, now string) {
		...
		query, args, err := binder.Default.In(q, telcodes)
		if err != nil {
			t.Error(err)
		}
...
func TestBindStruct(t *testing.T) {
	...
	bq, args, _ := bindStruct(binder.QUESTION, q1, am, mapper())
	expect := `INSERT INTO foo (a, b, c, d) VALUES (?, ?, ?, ?)`
	if bq != expect {
		t.Errorf("Interpolation of query failed: got `%v`, expected `%v`\n", bq, expect)
	}
	...
	am2 := tt2{"Hello", "World"}
	bq, args, _ = bindStruct(binder.QUESTION, "INSERT INTO foo (a, b) VALUES (:field_2, :field_1)", am2, mapper())
	...
	bq, args, err = bindStruct(binder.QUESTION, "INSERT INTO foo (a, b, c) VALUES (:name, :field_1, :field_2)", am3, mapper())
...
func BenchmarkBindStruct(b *testing.B) {
	...
	b.StartTimer()
	for b.Loop() {
		bindStruct(binder.DOLLAR, q1, am, mapper())
	}
}

func TestBindNamedMapper(t *testing.T) {
	type A map[string]any
	m := reflectx.NewMapperFunc("db", mpr.Name)
	query, args, err := bindNamedMapper(binder.DOLLAR, `select :x`, A{
		"x": "X!",
	}, m)
	if err != nil {
		t.Fatal(err)
	...
	_, _, err = bindNamedMapper(binder.DOLLAR, `select :x`, map[string]string{
		"x": "X!",
	}, m)
	if err == nil {
		t.Fatal("err is nil")
...
func BenchmarkIn(b *testing.B) {
	...
	for b.Loop() {
		_, _, _ = binder.Default.In(q, []any{"foo", []int{0, 5, 7, 2, 9}, "bar"}...)
	}
}
...
func BenchmarkIn1k(b *testing.B) {
	...
	for b.Loop() {
		_, _, _ = binder.Default.In(q, []any{"foo", vals[:], "bar"}...)
	}
}
...
func BenchmarkRebind(b *testing.B) {
	...
	defaultBinder := binder.Default
	for b.Loop() {
		defaultBinder.Rebind(binder.DOLLAR, q1)
		defaultBinder.Rebind(binder.DOLLAR, q2)
	}
}
...
func BenchmarkRebindBuffer(b *testing.B) {
	...
	defaultBinder := binder.Default
	for b.Loop() {
		defaultBinder.RebindBuff(binder.DOLLAR, q1)
		defaultBinder.RebindBuff(binder.DOLLAR, q2)
	}
}
...
func TestIn130Regression(t *testing.T) {
	...
	t.Run("[]any{}", func(t *testing.T) {
		q, args, err := defaultBinder.In("SELECT * FROM people WHERE name IN (?)", []any{[]string{"gopher"}}...)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("[]string{}", func(t *testing.T) {
		q, args, err := defaultBinder.In("SELECT * FROM people WHERE name IN (?)", []string{"gopher"})
		if err != nil {
			t.Fatal(err)
		}
		if q != "SELECT * FROM people WHERE name IN (?)" {
